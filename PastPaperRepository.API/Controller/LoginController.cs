...
public class LoginController : ControllerBase
{
    ...
    public async Task<IActionResult> Login([FromBody] LogInUser logInUser)
    {
        ...
        if (userData.IsUserDataComplete == 1) boolvalue = true;

        var claims = new List<Claim>
        {
            new("email", user.Email),
            new("role", "User"),
            new("userId", userData.Id.ToString()),
    ...
    {
        if (registerUser == null || string.IsNullOrWhiteSpace(registerUser.Name) ||
            string.IsNullOrWhiteSpace(registerUser.Email) ||
            string.IsNullOrWhiteSpace(registerUser.Password)) return BadRequest("Invalid user data provided.");
        ...
        if (!result) return StatusCode(500, "An error occurred while creating the user.");
    ...
    public async Task<IActionResult> SendUserData([FromBody] SendUserDataRequest request, CancellationToken token)
    {
        ...
        if (!result) return StatusCode(500, "An error occurred while saving the user data.");
